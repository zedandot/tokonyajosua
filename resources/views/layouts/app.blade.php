...
<head>
    ...
    {{-- Tambahan Turbolinks agar perpindahan halaman mulus tanpa reload putih --}}
...
<body class="bg-slate-50 text-slate-800 antialiased" x-data="{ sidebarOpen: false }">
    <div class="flex min-h-screen">
        ...
        <aside
            ...
            class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white flex flex-col shadow-xl
                   transform transition-transform duration-200 ease-in-out
                   lg:translate-x-0"
        >
            {{-- Logo --}}
            <div class="flex items-center gap-3 px-6 py-5 border-b border-slate-700">
                <div class="w-10 h-10 rounded-lg bg-sky-500 flex items-center justify-center font-bold text-lg shrink-0">S</div>
                <div>
                    <h1 class="font-bold text-lg leading-tight">SIMTOKO</h1>
                    <p class="text-xs text-slate-400">Retail Management</p>
                </div>
                {{-- Close button (mobile) --}}
                <button @click="sidebarOpen = false" class="ml-auto lg:hidden text-slate-400 hover:text-white" aria-label="Close sidebar">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('reports.index') }}" @click="sidebarOpen = false"
                   class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('reports.*') ? 'bg-sky-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                    Reports
                </a>
                <a href="{{ route('users.index') }}" @click="sidebarOpen = false"
                   class="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('users.*') ? 'bg-sky-600 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
        ...
        <div class="flex-1 flex flex-col min-w-0 lg:ml-64">
            ...
            {{-- Mobile Top Bar --}}
            <header class="sticky top-0 z-30 flex items-center gap-3 bg-white border-b border-slate-200 px-4 py-3 lg:hidden shadow-sm">
                <button @click="sidebarOpen = true" class="p-2 rounded-lg text-slate-600 hover:bg-slate-100 transition-colors" aria-label="Open menu">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
                </button>
                <div class="flex items-center gap-2">
                    <div class="w-7 h-7 rounded-md bg-sky-500 flex items-center justify-center font-bold text-sm text-white">S</div>
                    <span class="font-bold text-slate-800 text-sm">SIMTOKO</span>
                </div>
                <div class="ml-auto text-xs text-slate-400 hidden sm:block">@yield('title', 'Dashboard')</div>
            </header>
    ...
    {{-- Alpine.js — loaded here so all pages can use it --}}
